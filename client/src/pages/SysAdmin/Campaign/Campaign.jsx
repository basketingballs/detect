...
export default function Campaign() {
  ...
  const getCampaign = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/sysadmin/campaign`,
        options
      );
  ...
  const initCampaign = async () => {
    try {
      ...
      if (
        !buffer ||
        Math.floor((new Date() - new Date(buffer.created)) / (1000 * 60)) > 10
      ) {
        getCampaign();
      } else {
  ...
  const getUnits = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/sysadmin/campaign/unit`,
        options
      );
      const body = await response.json();
  ...
  const getLabs = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/sysadmin/unit/lab`,
        options
      );
  ...
  const getDoctors = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/sysadmin/unit/doc`,
        options
      );
      if (response.status == 200) {
        const body = await response.json();
              ...
              >
                <Dialog.Panel className="transform overflow-hidden rounded-2xl bg-white p-6 shadow-xl transition-all w-1/3">
                  <Dialog.Title
                    ...
                    className="text-lg font-medium leading-6 text-gray-900"
                  >
                    Unlink Laboratory {delRef.lab_name} from unit{" "}
                    {delRef.unit_name}
                  </Dialog.Title>
